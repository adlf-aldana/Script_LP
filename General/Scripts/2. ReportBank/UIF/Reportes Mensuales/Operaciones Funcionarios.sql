...
--> SE AUMENTO "decode(c.catrnndoc, null, c.catrnntra,c.catrnndoc)" PARA EVITAR NULOS - ZCE 02022024
...
and c.catrncorr=1
and c.catrnstat=0
and a.gbagecage in (select gbeplcemp from gbepl)
...
select c.catrnntra nro_trans,c.catrnftra fecha,'' num_cta_origen,b.camcancta num_cta_destino,a.gbagecage cod_cli,
		...
		TRIM(e.catmvdesc) desc_trans,c.catrnplaz suc,c.catrnagen agen,round(sum(c.catrnimpo*-1),2) imp_orig,c.catrncmon mon,
...
and c.catrnstat=0
and a.gbagecage in (select gbeplcemp from gbepl)
group by 1,2,4,5,6,7,8,10,11,12,14,16,17,18,19
--order by 2;
UNION
...
and d.gbagecage in (select gbeplcemp from gbepl)
UNION
...
and a.catrnftra between '{ini}' and '{fin}'
and f.gbagecage in (select gbeplcemp from gbepl)
UNION
--13 /*OPERACIONES - DPF*/
...
and c.catrncorr=1
and c.catrnstat=0
and a.uitrpcage in (select gbeplcemp from gbepl)
